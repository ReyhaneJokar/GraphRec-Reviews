...
def infer_checkpoint_edge_dim(state_dict):
    ...
    # Fallback
    for key in ("edge_attr_proj.1.weight", "edge_attr_proj.0.weight"):
        if key in state_dict:
            shape = tuple(state_dict[key].shape)
            if len(shape) == 2:
                return int(shape[1])
...
def infer_checkpoint_embedding_dim(state_dict):
    ...
    if key not in state_dict:
        raise RuntimeError("embedding.weight not found in checkpoint.")
...
def infer_checkpoint_num_layers(state_dict):
    ...
    if not layer_ids:
        raise RuntimeError("Could not infer GNN layer count from checkpoint.")
...
def load_text_only_edge_embeddings(project_dir: Path, embedding_file: Path, train_df: pd.DataFrame, negative_df: pd.DataFrame,):
    ...
    if not embedding_file.exists():
        raise FileNotFoundError(f"Text-only embedding file not found:\n{embedding_file}")

    embeddings = np.load(embedding_file)

    if embeddings.ndim != 2:
        raise ValueError(f"Embedding array must be 2D, got {embeddings.shape}")
    ...
    if embeddings.shape[0] <= max_needed:
        raise ValueError(f"Embedding file has {embeddings.shape[0]} rows, but max required row_index is {max_needed}.")

    pos_embeddings = embeddings[pos_rows].astype(np.float32, copy=False)
    neg_embeddings = embeddings[neg_rows].astype(np.float32, copy=False)
...
def main():
    ...
    ap.add_argument("--edge_attr_file", required=True, help="Explicit text-only review embedding file.")
    ...
    if embedding_dim != checkpoint_embedding_dim:
        raise RuntimeError(f"embedding_dim mismatch: checkpoint={checkpoint_embedding_dim}, requested={embedding_dim}")

    if layers != checkpoint_layers:
        raise RuntimeError(f"layers mismatch: checkpoint={checkpoint_layers}, requested={layers}")
    ...
    if missing:
        raise ValueError(f"LLM labels file missing columns: {sorted(missing)}")

    llm_df = llm_df[["row_index", "negative_confidence"]].copy()
    llm_df["row_index"] = llm_df["row_index"].astype(int)
    duplicate_count = llm_df["row_index"].duplicated().sum()

    if duplicate_count:
        raise ValueError(f"LLM labels contain {duplicate_count} duplicated row_index values. Each negative review must have at most one teacher label.")

    llm_df["negative_confidence"] = pd.to_numeric(llm_df["negative_confidence"], errors="coerce")
    llm_df = llm_df.dropna(subset=["negative_confidence"])
    llm_df["negative_confidence"] = llm_df["negative_confidence"].clip(0.0, 1.0)

    data, data_neg, _ = data_loader.data_loading(str(project_dir), load_val_or_test="val")
    ...
    neg_edge_index = data_neg["user", "rates", "item"].edge_index
    ...
    pos_edge_attr = torch.from_numpy(pos_review_raw_np)
    ...
    if not hasattr(data_h, "edge_attr") or data_h.edge_attr is None:
        raise RuntimeError("edge_attr missing after to_homogeneous().")

    homogeneous_edge_dim = int(data_h.edge_attr.shape[-1])

    if homogeneous_edge_dim != text_edge_dim:
        raise RuntimeError(f"Unexpected homogeneous edge dimension: {homogeneous_edge_dim}; expected {text_edge_dim}")
    ...
    model.eval()
    base_ckpt = torch.load(base_embeddings_path, map_location="cpu")

    if "user_emb" not in base_ckpt:
        raise RuntimeError("base_embeddings file does not contain 'user_emb'.")

    if "item_emb" not in base_ckpt:
        raise RuntimeError("base_embeddings file does not contain 'item_emb'.")

    base_user_emb = base_ckpt["user_emb"].float()
    base_item_emb = base_ckpt["item_emb"].float()

    if base_user_emb.shape != (num_users, embedding_dim):
        raise RuntimeError(
    ...
    if base_item_emb.shape != (num_items, embedding_dim):
        raise RuntimeError(
    ...
    print(f"Base user embedding: {tuple(base_user_emb.shape)}")
    print(f"Base item embedding: {tuple(base_item_emb.shape)}")

    with torch.no_grad():

        neg_review_raw = torch.from_numpy(neg_review_raw_np)
        pos_review_raw = torch.from_numpy(pos_review_raw_np)

        if model.edge_attr_proj is not None:
            e_r = model.compute_edge_feat_direction(neg_review_raw)
            pos_dirs = model.compute_edge_feat_direction(pos_review_raw)
        else:
            e_r = F.normalize(neg_review_raw, dim=-1)
            pos_dirs = F.normalize(pos_review_raw, dim=-1)

    centroid = torch.zeros(num_users, pos_dirs.size(-1), dtype=torch.float)
    counts = torch.zeros(num_users, dtype=torch.float)
    centroid.index_add_(0, pos_user_idx, pos_dirs)

    counts.index_add_(0, pos_user_idx, torch.ones(pos_user_idx.size(0), dtype=torch.float))
    counts = counts.clamp(min=1).unsqueeze(-1)
    centroid = F.normalize(centroid / counts, dim=-1)

    # Similarity of negative review to user's positive review centroid
    sim = (e_r * centroid[neg_user_idx]).sum(-1)
    user_pos_count_arr = np.bincount(pos_user_idx.numpy(), minlength=num_users)
    user_neg_count_arr = np.bincount(neg_user_idx.numpy(), minlength=num_users)
    item_pop_arr = np.bincount(pos_item_idx.numpy(), minlength=num_items)
    ...
    if rating_col is None:
        raise ValueError("train_edges.csv must contain rating:float or rating.")

    ratings = pd.to_numeric(pos_df[rating_col], errors="coerce").fillna(0.0).to_numpy()
    if len(ratings) != len(pos_item_idx):
        raise RuntimeError("Rating count does not match positive edge count.")

    item_rating_sum = np.zeros(num_items, dtype=np.float64)
    np.add.at(item_rating_sum, pos_item_idx.numpy(), ratings)
    item_avg_rating_arr = np.divide(item_rating_sum, np.maximum(item_pop_arr, 1))
    ...
    feats = pd.DataFrame(
        {
            ...
            "user_pos_count": z(user_pos_count_arr[neg_users_np]),
            "user_neg_count": z(user_neg_count_arr[neg_users_np]),
            "item_popularity": z(item_pop_arr[neg_items_np]),
            "item_avg_rating": z(item_avg_rating_arr[neg_items_np]),
        }
    )
    ...
    feats = pd.concat([feats.reset_index(drop=True), embedding_features], axis=1)
    feats = feats.merge(llm_df, on="row_index", how="left", validate="one_to_one").copy()
    feats["has_llm_label"] = (feats["negative_confidence"].notna())

    if len(feats) != len(neg_df):
        raise RuntimeError("Distillation dataset row count changed after LLM merge.")

    if feats["row_index"].duplicated().any():
        raise RuntimeError("Duplicate row_index detected in final distillation dataset.")

    print("=" * 80)
    print(f"Total negative edges: {len(feats)}")
    print(f"LLM-labeled negatives: {int(feats['has_llm_label'].sum())}")
    print(f"LLM label fraction: {feats['has_llm_label'].mean():.4f}")
